-- data_analyst_jobs (for reference)
...
FROM data_analyst_jobs
WHERE location IS NOT NULL AND location LIKE '__' AND star_rating IS NOT NULL
GROUP BY location
...
-- HAVING acts right after GROUP BY, so if you need an aggregate function before SELECT, use HAVING
...
SELECT DISTINCT domain AS industry, COUNT(skill) AS sql_jobs
FROM data_analyst_jobs
